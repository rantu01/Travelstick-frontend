...
import { deleteTestimonialsByAdmin, getAllTestimonialsByAdmin, updateTestimonialByAdmin } from "@/app/helper/backend";
...
const SiteTestimonials = () => {
  ...
  const [data, getData, { loading }] = useFetch(getAllTestimonialsByAdmin);
      ...
      <div className="w-full overflow-x-auto mt-7 dashboardModal">
        <div className=" rounded dashboardInput mx-6 bg-white">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#05073C] heading-3">Site Testimonials</h1>
            <BackButton />
          </div>
          {
            langCode && (
              <Table
                ...
                loading={loading}
                onReload={getData}
                onDelete={deleteTestimonialsByAdmin}
                indexed
                langCode={langCode}
                pagination
              />
