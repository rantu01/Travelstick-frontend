...
const Package = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataField: "check_in",
      formatter: (_, d) => (
        <span>{dayjs(d?.check_in).format("DD MMM YYYY")}</span>
      ),
    },
    {
      text: "Check Out",
      dataField: "check_out",
      formatter: (_, d) => (
        <span>{dayjs(d?.check_out).format("DD MMM YYYY")}</span>
      ),
    },
    {
      text: i18n.t("Status"),
      ...
      <div className="w-full overflow-x-auto mt-7 px-6">
        <div className="rounded dashboardInput bg-white">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#05073C] heading-3">{i18n.t("Package List")}</h1>
            <BackButton />
          </div>
          <Table
            columns={columns}
            ...
            action={
              <Button
                onClick={() => {
                  router.push("/admin/packageManagement/package/addPackage");
                }}
              >
                {i18n.t("Add New")}
              </Button>
            ...
            onEdit={(values) => {
              router.push(
                `/admin/packageManagement/package/editPackage?_id=${values._id}`
              );
            }}
            // onView={handleView}
            onView={(value) => {
              router.push(
                `/admin/packageManagement/package/view?_id=${value._id}`
              );
            }}
