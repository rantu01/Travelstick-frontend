...
const Room = () => {
  ...
  const [data, getData, { loading }] = useFetch(getAllRooms);
  const i18n = useI18n();
  ...
  const columns = [
    {
      ...
      formatter: (value) => (
        <>
          <TableImage url={value?.[0] || "/man.png"} />
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      formatter: (hotel) => (
        <span>
          <Tooltip
            title={
              hotel?.name?.[langCode]?.length > 40 ? hotel?.name?.[langCode] : undefined
            }
          >
            <span className="cursor-help">
              {hotel?.name?.[langCode]?.length > 40
                ? hotel?.name?.[langCode]?.slice(0, 40) + "..."
                : hotel?.name?.[langCode]}
            </span>
          </Tooltip>
        </span>
      ...
      <div className="w-full overflow-x-auto mt-7 px-6">
        <div className="rounded dashboardInput bg-white">
          <div className="flex justify-between px-8 pt-8 items-center">
            <h1 className="text-[#05073C] heading-3">{i18n.t("Room List")}</h1>
            <BackButton />
          </div>
          <Table
            columns={columns}
            ...
            action={
              <Button
                onClick={() => {
                  router.push("/admin/hotelManagement/room/addRoom");
                }}
              >
                {"Add New"}
              </Button>
            ...
            onEdit={(values) => {
              router.push(
                `/admin/hotelManagement/room/editRoom/${values._id}`
              );
            }}
            onDelete={deleteRoom}
            indexed
