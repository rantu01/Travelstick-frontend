...
const Table = ({
  ...
  return (
    <>
      <div
        className={`w-full xl:p-[30px] lg:p-6 md:p-5 sm:p-4 p-3 rounded-sm mb-4 ${pathname === "/vendor" ? "seller" : ""}`}
      >
        {noHeader || (
          <header className="pb-6  border-b-[1px] gap-3 border-primary/80  flex justify-between flex-wrap">
            {title ? (
              <>
                {typeof title === "string" ? (
              ...
              <div className="flex justify-between">
                <div className="flex flex-wrap">
                  <SearchInput
                    className="w-44 border-[1px] border-primary focus:outline-primary"
                    onChange={(e) => {
        ...
        >
                ...
                {loading ? (
                  ...
                  <>
                    {(pagination ? data?.docs : data)?.map((row, index) => (
                      <tr key={index}>
                        {indexed && (
                          <td className="py-6 whitespace-nowrap text-[#05073C]">
                        ...
                        {cols?.map((column, index) => (
                          <td
                            className={`p-2 whitespace-nowrap text-[#05073C] ${
                              column?.className || ""
                            }`}
                            key={index}
                          >
          ...
          {pagination && (
            <div className="pt-5 border-t-[1px] border-primary/80">
              <Pagination
                page={data?.page}
  ...
  return (
    <div className="w-inline-block h-10 w-10 ">
      <Image
        ...
        style={{ "maxinlineSize": "100%", "maxblockSize": "100%" }}
      />
      <Modal
        width={800}
        open={image}
