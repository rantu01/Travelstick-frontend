...
import { createHotelBookingPayment, getAllPublicHotelServices, hotelBookingCalculation } from "@/app/helper/backend";
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import HotelCounterButton from "../btn/hotelCounterButton";
import AuthModal from "../site/common/component/authModal";
const HotelBooking = ({ data, user }) => {
  ...
  const handleHotelServicePrice = (serviceId) => async (e) => {
  ...
  const handlePayment = async () => {
    ...
    const response = await createHotelBookingPayment({ body: payload });
  ...
  useEffect(() => {
    if (check_in && check_out) {
      handleHotelBookingCalculation();
    }
  }, [check_in, check_out]);


  const handleHotelBookingCalculation = async () => {
    const hotelData = await hotelBookingCalculation({
      body: {
        hotel: data?._id,
        person: count,
  ...
  return (
    <div className="w-full relative ">
            ...
            <div className="flex justify-between items-center">
              <div className="border-r flex-1 lg:p-4 md:p-3 p-2">
                ...
                <div className="mt-3">
                  ...
                  >
                    <DatePicker
                      onChange={(e) => setCheckIn(e ? e.format("YYYY-MM-DD") : null)}
                      allowClear
              ...
              <div className="flex-1 lg:p-4 md:p-3 p-2">
                ...
                <div className="mt-3">
                  ...
                  >
                    <DatePicker
                      onChange={(e) => setCheckOut(e ? e.format("YYYY-MM-DD") : null)}
                      allowClear
                      showToday={false}
                      disabledDate={(current) =>
                        current && (
                          current < dayjs().startOf("day") ||
                          (check_in && current <= dayjs(check_in).startOf("day"))
                        )
                      }
                      format="YYYY-MM-DD"
                    ...
                    check_in && check_out && (
                      <div>
                        <HotelCounterButton
                          ...
                          id={data?._id}
                          hotelBookingCalculation={hotelBookingCalculation}
      ...
      >
        ...
        <button className="xl:mt-8 lg:mt-6 mt-5 w-full text-white common-btn !rounded-md"
          onClick={() => handlePayment()}
        >
          {i18n.t('Pay Now')}
        </button>
      </Modal>
      <AuthModal authModalOpen={authModalOpen} slug={`/hotel/${data?._id}`} setAuthModalOpen={setAuthModalOpen} />
    </div>
  )
}
