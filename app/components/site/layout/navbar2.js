...
import { MdArrowOutward, MdOutlineShoppingCart } from "react-icons/md";
import NotificationDropdown from "../../common/notification";

const Navbar2 = ({ textColor = "text-white" }) => {
  const [setting] = useFetch(fetchPublicSettings);
  const [notifications, getNotifications] = useFetch(getNotificationsByAdmin, {
    limit: 100,
  });
  ...
  return (
    <div className="w-full font-lato overflow-hidden relative z-50">
      <div className="travel-container flex flex-row justify-between items-center lg:py-8 py-4 lg:px-0">
        ...
        {/* Desktop Menu */}
        <ul className="list-none xl:gap-4 gap-2 items-center hidden lg:flex navbarbg2 px-4 py-3">
          <NavItem
            path="/"
            label={i18n.t("Home")}
            pathname={pathname}
        ...
        <div className="flex gap-0 sm:gap-2 items-center">
          ...
          {user?._id && (
            <div className="cursor-pointer flex mr-3  items-center justify-center rounded-full">
              ...
              >
                ...
                >
                  <IoNotificationsOutline
                    className={`text-xl lg:text-[22px]  ${textColor}`}
                  />
                </Badge>
              </Popover>
            </div>
                  ...
                  <div className="ml-2 sm:ml-0">
                    {user?.image ? (
                      <Image
                        ...
                        height={40}
                        alt="user"
                        className="border w-[25px] h-[25px] md:w-[30px] md:h-[30px] lg:w-[40px] lg:h-[40px] rounded-full object-contain"
                      />
                    ) : (
          ...
          <Link href="/package">
            <div className="hidden xl:flex gap-4 items-center navbarbg2 py-2 px-6 rounded-full ">
              <p className="text-base hidden xl:block whitespace-pre font-medium text-white hover:text-primary ">
                {i18n?.t("Book Now")}
              </p>

              <div className="bg-primary hover:bg-[#EB662B] p-3 rounded-full">
