...
import { getAllHotelBookingByUser, } from "@/app/helper/backend";
...
const HotelBooking = () => {
  ...
  const [data, getData, { loading }] = useFetch(getAllHotelBookingByUser, {}, false);
  useEffect(() => {
    getData();
  }, []);
  ...
  return (
    <>
      <div className="w-full overflow-x-auto">
        <h4 className="description-3 mb-6 text-[#05073C]">{i18n?.t("Hotel Booking")}</h4>
        {
          langCode && (
            <UserTable
              columns={columns}
       ...
       <HotelBookingModal slug={"user"} viewModalOpen={viewModalOpen} setViewModalOpen={setViewModalOpen} viewData={viewData} setViewData={setViewData} />
    </>
  );
};
