...
const PackageBookingPage = () => {
  ...
  const [bookingData, getData, { loading }] = useFetch(getAllPackageBookingByUser, { limit: 5 });
  const data = bookingData?.docs
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (value) => (
        <Tooltip title={
          value?.name[langCode]?.length > 20 ? value?.name?.[langCode] : undefined}>
  ...
  return (
    <>
      <div className="w-full overflow-x-auto">
        <h4 className="description-3  text-[#05073C]">{i18n?.t("Recent Package Bookings")}</h4>
        {
          langCode && (
            <UserTable
              title=" "
